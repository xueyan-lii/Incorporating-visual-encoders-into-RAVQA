#only difference from okvqa_datasets is clip embedding is loaded from question_id rather than img_key line 75
import os
import re
import sys
import time
...
class OKVQADataset_Instruct(torch.utils.data.Dataset, ModuleParser):
    """
    ...
    def __getitem__(self, idx):
        ...
        clip_embeddings = self.clip_embeddings.get(str(item.question_id), None)
        if clip_embeddings == None:
            print('cannot find question_id', item.question_id)
